...
void main () {
	...
	//rotate particles
	vec3 world_pos = rotation3dY(time / 3. * 0.3 * (0.1 + 0.5 * particle_size) ) * position;
	...
	float offsetY = sin(position.x + time / 1.2) * 0.3;

	vec3 particle_position = (modelMatrix * vec4(world_pos + offset + offset0, offsetY)).xyz;
